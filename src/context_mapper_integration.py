...
class ContextMapperIntegration:
    ...
    def __init__(self):
        ...
        self.tools = {
            'contextmapper': ContextMapperTool(
                name='Context Mapper CLI',
                command='contextmapper'
            ),
            'cml': ContextMapperTool(
                name='CML CLI',
                command='cml'
            ),
    ...
    def _detect_tools(self) -> None:
        ...
        for tool_id, tool in self.tools.items():
            try:
                ...
                if result.returncode == 0 or tool_id == 'java':  # Java returns non-zero for -version
                    tool.available = True
    ...
    def validate_cml_with_cli(self, cml_code: str) -> ValidationResult:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.cml', delete=False) as temp_file:
    ...
    def generate_artifacts(self, cml_code: str, generator_type: str = "plantuml") -> ValidationResult:
        """
    ...
    def _get_setup_recommendations(self) -> List[str]:
        ...
        if not self.tools['java'].available:
            recommendations.append("Install Java Runtime Environment (JRE) 11 or higher")
        
        if not self.tools['contextmapper'].available:
            recommendations.append("Install Context Mapper CLI from https://contextmapper.org/docs/cli/")
...
class ContextMapperWorkflow:
    ...
    def validate_and_generate_workflow(self, json_data: Dict[str, Any], generators: List[str] = None) -> Dict[str, ValidationResult]:
        """
        ...
        try:
            ...
            results['conversion'] = ValidationResult(is_valid=True, errors=[], warnings=[])
        except Exception as e:
