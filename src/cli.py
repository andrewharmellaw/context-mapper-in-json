...
@click.option('--validate-only', '-v', is_flag=True, help='Only validate JSON, do not convert')
@click.option('--skip-cml-validation', is_flag=True, help='Skip CML output validation')
@click.option('--enable-round-trip', is_flag=True, help='Enable round-trip validation (Phase 4)')
@click.option('--use-context-mapper-cli', is_flag=True, help='Use Context Mapper CLI for validation (Phase 4)')
@click.option('--generate-artifacts', multiple=True, help='Generate artifacts using Context Mapper (plantuml, mdsl, etc.)')
@click.option('--verbose', is_flag=True, help='Enable verbose logging')
@click.option('--format-output', is_flag=True, default=True, help='Format CML output (default: True)')
def convert(
    input_file: Path,
    ...
    try:
        ...
        if enable_round_trip and round_trip_validator:
            ...
            round_trip_result = round_trip_validator.validate_round_trip(json_data, cml_output)
        ...
        if generate_artifacts and context_mapper_integration:
            ...
            for generator in generate_artifacts:
                ...
                gen_result = context_mapper_integration.generate_artifacts(cml_output, generator)
        ...
        relationship_count = len(json_data.get("contextMap", {}).get("relationships", []))
        aggregate_count = sum(len(bc.get("aggregates", [])) for bc in json_data.get("boundedContexts", []))
...
def validate(input_file: Path):
    ...
    convert.callback(input_file, None, True, False, False, True)
...
@main.command()
@click.argument('input_file', type=click.Path(exists=True, path_type=Path))
@click.option('--generator', '-g', multiple=True, default=['plantuml'], 
