...
from src.validation import ValidationResult, ValidationError
...
class CMLParser:
    ...
    def _setup_patterns(self) -> None:
        ...
        self.entity_pattern = re.compile(
            r'Entity\s+(\w+)(?:\s+aggregateRoot)?\s*\{'
        )
        ...
        # Operation pattern
        self.operation_pattern = re.compile(r'(?:(public|private|protected)\s+)?(\w+)\s+(\w+)\(([^)]*)\)')
        
        # Relationship patterns
        self.partnership_pattern = re.compile(r'(\w+)\s+Partnership\s+(\w+)')
        self.shared_kernel_pattern = re.compile(r'(\w+)\s+\[SK\]\s+<->\s+\[SK\]\s+(\w+)(?:\s*:\s*(.+))?')
        self.customer_supplier_pattern = re.compile(r'(\w+)\s+\[([^\]]+)\]->\[([^\]]+)\]\s+(\w+)(?:\s*:\s*([^{]+))?(?:\s*\{\s*([^}]+)\s*\})?')
    ...
    def _parse_context_map(self, lines: List[str]) -> Optional[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            if in_context_map:
                ...
                if contains_match:
                    contexts = [ctx.strip() for ctx in contains_match.group(1).split(',')]
                ...
                if cs_match:
                    upstream, upstream_roles, downstream_roles, downstream, tech, aggregates = cs_match.groups()
                    rel = {
                    ...
                    if upstream_roles:
                        rel['upstreamRoles'] = [r.strip() for r in upstream_roles.split(',')]
                    if downstream_roles:
                        rel['downstreamRoles'] = [r.strip() for r in downstream_roles.split(',')]
                    if tech:
                        rel['implementationTechnology'] = tech.strip()
                    if aggregates:
                        rel['exposedAggregates'] = [a.strip() for a in aggregates.split(',')]
                    context_map['relationships'].append(rel)
...
class RoundTripValidator:
    ...
    def validate_round_trip(self, original_json: Dict[str, Any], generated_cml: str) -> ValidationResult:
        """
        ...
        try:
            ...
            discrepancies = self.compare_semantic_equivalence(original_json, parsed_json)
            
            # Step 3: Convert discrepancies to validation errors
            for discrepancy in discrepancies:
                if discrepancy.discrepancy_type in ["MISSING_PROPERTY", "TYPE_MISMATCH", "VALUE_MISMATCH"]:
            ...
            logger.info(f"Round-trip validation completed with {len(result.errors)} errors and {len(result.warnings)} warnings")
    ...
    def compare_semantic_equivalence(self, original: Dict[str, Any], parsed: Dict[str, Any]) -> List[Discrepancy]:
        """
        ...
        if 'contextMap' in original:
            if 'contextMap' not in parsed:
                ...
                discrepancies.extend(self._compare_context_map(
                    original['contextMap'], 
                    parsed['contextMap']
                ))
        ...
        if 'boundedContexts' in original:
            if 'boundedContexts' not in parsed:
                ...
                discrepancies.extend(self._compare_bounded_contexts(
                    original['boundedContexts'], 
                    parsed['boundedContexts']
                ))
    ...
    def _compare_context_map(self, original: Dict[str, Any], parsed: Dict[str, Any]) -> List[Discrepancy]:
    ...
    def _compare_bounded_contexts(self, original: List[Dict[str, Any]], parsed: List[Dict[str, Any]]) -> List[Discrepancy]:
    ...
    def identify_discrepancies(self, json_data: Dict[str, Any], cml_data: Dict[str, Any]) -> List[Discrepancy]:
        """
    ...
    def validate_with_context_mapper(self, cml_code: str) -> ValidationResult:
        ...
        try:
            ...
            valid_keywords = [
                'ContextMap', 'BoundedContext', 'Aggregate', 'Entity', 'ValueObject',
                'DomainEvent', 'Command', 'Service', 'Repository', 'Domain', 'Subdomain'
            ]
            ...
            for i, line in enumerate(lines, 1):
                ...
                if line and not line.startswith('//'):
                    ...
                    words = line.split()
                    if words and words[0] not in valid_keywords and '=' not in line and not line.endswith('{') and not line.endswith('}'):
