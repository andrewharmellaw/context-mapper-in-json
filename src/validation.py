...
from schemas.bounded_context import BOUNDED_CONTEXT_SCHEMA, BOUNDED_CONTEXT_SEMANTIC_RULES
...
class ValidationEngine:
    ...
    def validate_json_schema(self, json_data: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            if "boundedContexts" in json_data:
                self._validate_bounded_contexts_schema(json_data["boundedContexts"], result)
    ...
    def _validate_context_map_schema(self, context_map_data: Dict[str, Any], result: ValidationResult) -> None:
    ...
    def _validate_bounded_contexts_schema(self, bounded_contexts_data: List[Dict[str, Any]], result: ValidationResult) -> None:
    ...
    def _validate_subdomains_schema(self, subdomains_data: List[Dict[str, Any]], result: ValidationResult) -> None:
    ...
    def _validate_complete_structure_schema(self, json_data: Dict[str, Any], result: ValidationResult) -> None:
    ...
    def validate_semantic_rules(self, json_data: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            if "contextMap" in json_data and "boundedContexts" in json_data:
                self._validate_context_map_semantics(json_data["contextMap"], json_data["boundedContexts"], result)
                self._validate_bounded_context_semantics(json_data["boundedContexts"], result)
    ...
    def _validate_context_map_semantics(self, context_map: Dict[str, Any], bounded_contexts: List[Dict[str, Any]], result: ValidationResult) -> None:
    ...
    def _validate_bounded_context_semantics(self, bounded_contexts: List[Dict[str, Any]], result: ValidationResult) -> None:
    ...
    def validate_references(self, json_data: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            if "contextMap" in json_data and "boundedContexts" in json_data:
                self._validate_cross_references(json_data["contextMap"], json_data["boundedContexts"], result)
    ...
    def _validate_cross_references(self, context_map: Dict[str, Any], bounded_contexts: List[Dict[str, Any]], result: ValidationResult) -> None:
    ...
    def _get_schema_error_suggestion(self, error: JsonSchemaValidationError) -> Optional[str]:
        """Generate helpful suggestions for schema validation errors."""
        if "is not of type" in error.message:
            return f"Check the data type - expected {error.schema.get('type', 'unknown')}"
        ...
        for error in errors:
            if error.suggestion:
                ...
                if error.error_type == "SCHEMA_ERROR":
                    ...
                    suggestions.append("Verify that all referenced elements are defined")
