...
class CMLValidator:
    ...
    def _setup_patterns(self) -> None:
        ...
        self.valid_business_models = {"REVENUE", "ENGAGEMENT", "COMPLIANCE", "COST_REDUCTION"}
            ...
            try:
                ...
                if current_element and brace_count > 0:
                    ...
                    if enum_property_match:
                        ...
                        if prop_name == "knowledgeLevel" and prop_value not in self.valid_knowledge_levels:
                            result.add_error(ValidationError(
    ...
    def _validate_parsed_elements(self, elements: List[CMLElement], result: ValidationResult) -> None:
        ...
        if len(bc_names_list) != len(set(bc_names_list)):
            duplicates = [name for name in bc_names_list if bc_names_list.count(name) > 1]
            result.add_error(ValidationError(
